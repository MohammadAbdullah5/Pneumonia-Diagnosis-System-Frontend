...
const LoginRequests = () => {
  ...
  useEffect(() => {
    ...
    const fetchLoginAttempts = async () => {
      try {
        const response = await axios.get('https://localhost:7098/api/admin/login-attempts');
        setLoginAttempts(response.data);
        setLoading(false);
      } catch (error) {
        console.error('There was an error fetching the login attempts!', error);
      }
    };
  ...
  const unflagIp = async (ipAddress) => {
    try {
      await axios.post('https://localhost:7098/api/admin/unflag-ip', { ipAddress });
  ...
  <div className="max-w-4xl mx-auto">
    <h1 className="text-2xl font-bold text-blue-600 mb-4">Login Attempt Logs</h1>
    ...
    <div className="bg-blue-600 text-white text-sm font-mono rounded-md p-2 h-80 overflow-y-auto shadow-inner">
      <div className="min-w-[600px] w-full overflow-x-auto space-y-1">
        {loginAttempts.map((attempt) => (
          ...
          >
            <span className={`pr-2 ${attempt.isSuccessful ? 'text-green-400' : 'text-red-400'}`}>
              [{attempt.isSuccessful ? 'SUCCESS' : 'FAILED'}]
            </span>
            <span className="flex-1 text-white/80 px-2 truncate">
              {new Date(attempt.attemptTime).toLocaleString()} | {attempt.email} | {attempt.ipAddress}
            </span>
            {attempt.IsFlagged && (
              <button
